...
def build_phaser_html(cfg: GameConfig) -> str:
    ...
    # Keep everything inline and asset-free; Phaser from CDN.
    html = f"""<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
...
SYSTEM_JSON = (
    ...
    + ". Values must be valid for a two-stage, guided card mini-game in Phaser. "
      "Colors must be 7-char hex like #RRGGBB. Sequences are comma-separated like 'K,Q,4,5,6,7,8'. "
      "Keep tutorial_delay_ms between 0 and 8000. Use a short, punchy title and CTA."
)

def try_parse_json(text: str) -> Optional[Dict[str, Any]]:
    # If the model accidentally wrapped JSON in code fences, strip them.
...
with left:
    ...
    st.write("Tell the copilot what you want (theme, mood, languages, sequences, timing, colors, CTA, etc.).")
    ...
    user_prompt = st.text_area(
        ...
        "Hindi+English hints; warm colors; energetic CTA; tutorial hand after 3s.",
        height=120
    )
...
with colx:
    if st.button("Download index.html"):
        ...
        st.markdown(f"[Download the HTML](sandbox:{path})")
with coly:
    if st.button("Download design.json"):
        path_json = "/mnt/data/design.json"
        with open(path_json, "w", encoding="utf-8") as f:
            f.write(json.dumps(asdict(st.session_state.cfg), indent=2))
        st.success("Exported design.json")
        st.markdown(f"[Download the JSON](sandbox:{path_json})")
